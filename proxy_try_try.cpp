...
std::ofstream LogInfo("proxy.log");
        ...
        if(response->find(http::field::cache_control) != response->end()){
            ...
            if(response->find(http::field::expires)== response->end()){
                ...
                if (expire > now){
                	...
                	LogInfo<<"In cache, but expired at "<<expire_str<<std::endl;
                    return false;
                }
            } 
        }
